...
import { deviceTag, studentName, studentRecords, studentSubjects } from "@/lib/attendance-types";
...
function AnimatedCollegeCrest({ size = 88 }: { size?: number }) {
  return <View style={[styles.crestAnimation, { width: size, height: size, shadowColor: colors.gold, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.6, shadowRadius: 20, elevation: 15, borderRadius: size / 2, backgroundColor: "rgba(255, 215, 0, 0.05)" }]}><Image accessibilityLabel="Institute of Civil and Rural Engineering college crest" source={collegeCrest} style={styles.crestImage} resizeMode="contain" /></View>;
}
...
export default function HomeScreen() {
  ...
  const [studentDeviceTag, setStudentDeviceTag] = useState(deviceTag);
  ...
  if (screen === "studentLogin") return <StudentAccess onBack={() => open("welcome")} onAuthenticated={(result) => { setRole("student"); setAccountName(result.fullName); setStudentClassDivision(result.classDivision || ""); setStudentDeviceTag(result.deviceTag ?? deviceTag); setDeviceVerified(result.deviceVerified); open("deviceSetup"); }} onRegister={() => open("studentRegister")} />;
  if (screen === "studentRegister") return <StudentRegistration onBack={() => open("studentLogin")} onRegistered={(result) => { setRole("student"); setAccountName(result.fullName); setStudentClassDivision(result.classDivision || ""); setStudentDeviceTag(result.deviceTag ?? deviceTag); setDeviceVerified(false); open("deviceSetup"); }} />;
  ...
  return <ScreenContainer containerClassName="bg-background">
    ...
    <ScrollView style={styles.flex} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {screen === "liveSession" ? <LiveSession scanState={scanState} detectedCount={detectedCount} scanError={scanError} matchedStudents={[...matchedStudents, ...manualLinks]} unmatchedTags={discoveredTags.filter((tag) => !matchedStudents.some((student) => student.deviceTag === tag) && !manualLinks.some((student) => student.deviceTag === tag))} onLinkUnmatched={(tag: string) => Alert.alert("Manually link device", `Assign ${tag} to a student for this attendance review. This does not change the student’s saved device tag.`, [{ text: "Cancel", style: "cancel" }, ...[{ fullName: "Aarav Patil", enrollmentNumber: "21CE045" }, { fullName: "Isha Deshmukh", enrollmentNumber: "21CE051" }, { fullName: "Rohan Jadhav", enrollmentNumber: "21CE058" }].map((student) => ({ text: student.fullName, onPress: () => { setManualLinks((links) => [...links, { ...student, deviceTag: tag, manuallyLinked: true }]); setScanError(`Manual review: ${tag} linked to ${student.fullName} for this attendance session.`); } }))])} onScan={async () => {
        ...
        Alert.alert("Attendance locked", "Review discovered students and manual overrides before the final server-connected submission.");
        // Try to close session 1 for demo purposes and send SMS
        try {
...
function Welcome({ onStudent, onAdmin }: { onStudent: () => void; onAdmin: () => void }) {
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}>
    ...
    <View style={styles.welcomeActions}>
      ...
      <Pressable disabled={backendTest.isFetching} onPress={() => { void backendTest.refetch(); }} style={({ pressed }) => [styles.backendTestCard, backendTest.isFetching && styles.disabledCard, pressed && !backendTest.isFetching && styles.pressed]}><RoundIcon name="cloud-done" background="#EAF0F8" color={colors.navy} /><View style={styles.flex}><Text style={styles.roleName}>{backendTest.isFetching ? "Testing backend connection…" : "Test backend connection"}</Text><Text style={styles.roleDetail}>Checks the live API and database safely without changing attendance data.</Text></View>{backendTest.isFetching ? <ActivityIndicator color={colors.navy} /> : <MaterialIcons name="play-circle-outline" size={24} color={colors.navy} />}</Pressable>
      {backendTest.data ? <Callout tone={backendTest.data.database === "connected" ? "success" : "error"} text={backendTest.data.database === "connected" ? "Backend test passed: the API is online and the attendance database is connected." : "The API is online, but the database is not reachable. Check the server database configuration before testing attendance."} /> : null}
      <Text style={styles.credit}>Developed by Vinayraj Kore · v1.0</Text>
    </View>
    </ScrollView>
  </ScreenContainer>;
}
  ...
  return <>
    <View style={[styles.studentHero, { shadowColor: colors.navy, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.28, shadowRadius: 16, elevation: 8 }]}>
      <View>
        ...
        <Text style={styles.bigPercent}>82.4%</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 4 }}>
          <MaterialIcons name="trending-up" size={13} color="#4CAF87" />
          <Text style={styles.heroMeta}>Overall attendance · On track</Text>
        </View>
      </View>
      <View style={styles.daysRing}><Text style={styles.daysNumber}>26</Text><Text style={styles.daysLabel}>DAYS</Text></View>
    </View>
    ...
    <View style={styles.sessionCard}><View style={styles.sessionIcon}><MaterialIcons name="sensors" size={22} color="#FFFFFF" /></View><View style={styles.flex}><Text style={styles.sessionEyebrow}>LIVE NOW · CS-301</Text><Text style={styles.sessionTitle}>Data Structures attendance is open</Text><Text style={styles.sessionMeta}>Room 204 · closes in 08 min</Text></View></View>
    ...
    {studentSubjects.map((subject) => <View style={styles.subjectRow} key={subject.code}><View style={[styles.dot, { backgroundColor: subject.color }]} /><View style={styles.flex}><Text style={styles.subjectName}>{subject.name}</Text><Text style={styles.smallMuted}>{subject.code} · {subject.attended}/{subject.total} lectures</Text></View><Text style={styles.subjectPct}>{Math.round((subject.attended / subject.total) * 100)}%</Text></View>)}
    <Pressable onPress={() => onOpen("notices")} style={({ pressed }) => [styles.noticePrompt, pressed && styles.pressed]}><RoundIcon name="notifications-none" background="#F8EDEF" color={colors.maroon} /><View style={styles.flex}><Text style={styles.promptTitle}>2 unread notices</Text><Text style={styles.smallMuted}>Department lab schedule and timetable update.</Text></View><Text style={styles.titleAction}>Open</Text></Pressable>
  </>;
}
...
function Notices() { return <><Notice accent={colors.gold} date="TODAY · 09:10" title="Data Structures lab venue updated" body="Today’s lab section will meet in Computer Lab 2. Please arrive five minutes early." author="Posted by Prof. Meera Kulkarni" /><Notice accent={colors.maroon} date="17 AUG 2026" title="Department seminar attendance" body="All Computer Department students are requested to attend the guest lecture on Friday." author="Posted by Computer Department" /></>; }
...
function StartSession({ accountName, onStart }: { accountName: string; onStart: () => void }) {
  ...
  const catalog = catalogQuery.data?.map((item) => ({ name: item.name, teacherName: item.teacherName, classDivision: item.classDivision, room: item.room, startTime: item.startTime })) ?? [{ name: "Data Structures", teacherName: "Prof. Meera Kulkarni", classDivision: "TY Computer A", room: "Room 204", startTime: "10:30" }, { name: "Database Systems", teacherName: "Prof. Anil Deshmukh", classDivision: "TY Computer B", room: "Room 102", startTime: "13:30" }];
  const [subject, setSubject] = useState("Data Structures");
  const [teacherName, setTeacherName] = useState(accountName || "Prof. Meera Kulkarni");
  const [classDivision, setClassDivision] = useState("TY Computer A");
...
function ManageInfo({ accountName }: { accountName: string }) {
  ...
  const fallbackItems: ManagedSubject[] = [{ id: "cs301", subject: "Data Structures", code: "CS-301", teacher: "Prof. Meera Kulkarni", classDivision: "TY Computer A", room: "Room 204", timeSlot: "10:30–11:30", dayOfWeek: 1 }, { id: "cs302", subject: "Database Systems", code: "CS-302", teacher: "Prof. Anil Deshmukh", classDivision: "TY Computer B", room: "Room 102", timeSlot: "13:30–14:30", dayOfWeek: 1 }];
  ...
  const items = databaseItems ?? fallbackItems;
...
function AdminRecords() {
  ...
  const report: AttendanceReport = { date: "18 Aug 2026", subject: "Data Structures", classDivision: "TY Computer A", rows: [{ student: "Aarav Patil", enrollmentNumber: "21CE045", status: "Present", method: "Bluetooth" }, { student: "Isha Deshmukh", enrollmentNumber: "21CE051", status: "Present", method: "Bluetooth" }, { student: "Rohan Jadhav", enrollmentNumber: "21CE058", status: "Manual", method: "Faculty override" }, { student: "Neha Shinde", enrollmentNumber: "21CE061", status: "Absent", method: "Not marked" }] };
  const fallbackDailyReport: DailyAttendanceReport = { date: "18 Aug 2026", lectures: [{ subject: "Data Structures", teacherName: "Prof. Meera Kulkarni", classDivision: "TY Computer A", room: "Room 204", timeSlot: "10:30–11:30", absentStudents: [{ name: "Neha Shinde", enrollmentNumber: "21CE061" }] }, { subject: "Database Systems", teacherName: "Prof. Anil Deshmukh", classDivision: "TY Computer B", room: "Room 102", timeSlot: "13:30–14:30", absentStudents: [{ name: "Vikram Pawar", enrollmentNumber: "21CE073" }, { name: "Sana Kulkarni", enrollmentNumber: "21CE077" }] }] };
  ...
  const dailyReport: DailyAttendanceReport = filteredAbsences ? { date: `${startDate} to ${endDate}`, lectures: Object.values(filteredAbsences.reduce<Record<string, DailyAttendanceReport["lectures"][number]>>((groups, row) => { const key = `${row.date}-${row.subject}-${row.startTime}`; const existing = groups[key] ?? { subject: `${row.subject} · ${row.date}`, teacherName: row.teacherName, classDivision: row.classDivision, room: row.room, timeSlot: `${row.startTime}–${row.endTime}`, absentStudents: [] }; existing.absentStudents.push({ name: row.studentName, enrollmentNumber: row.enrollmentNumber }); groups[key] = existing; return groups; }, {})) } : fallbackDailyReport;
  const exportPdf = () => { void shareAttendancePdf(report).catch((error) => Alert.alert("PDF export unavailable", error instanceof Error ? error.message : "Unable to create the attendance PDF")); };
  const exportDailyPdf = () => { void shareDailyAttendancePdf(dailyReport).catch((error) => Alert.alert("Daily PDF export unavailable", error instanceof Error ? error.message : "Unable to create the full-day PDF")); };
  const exportCsv = () => { void shareAttendanceCsv(report).catch((error) => Alert.alert("CSV export unavailable", error instanceof Error ? error.message : "Unable to create the attendance CSV")); };
  return <><View style={styles.rangeRow}><View style={styles.flex}><Text style={styles.rangeLabel}>Report from</Text><TextInput value={startDate} onChangeText={setStartDate} style={styles.rangeInput} placeholder="YYYY-MM-DD" placeholderTextColor="#98A2B3" /></View><View style={styles.flex}><Text style={styles.rangeLabel}>Report to</Text><TextInput value={endDate} onChangeText={setEndDate} style={styles.rangeInput} placeholder="YYYY-MM-DD" placeholderTextColor="#98A2B3" /></View></View><View style={styles.rangeRow}><View style={styles.flex}><Text style={styles.rangeLabel}>Class filter</Text><TextInput value={classFilter} onChangeText={setClassFilter} style={styles.rangeInput} placeholder="All classes" placeholderTextColor="#98A2B3" /></View><View style={styles.flex}><Text style={styles.rangeLabel}>Subject filter</Text><TextInput value={subjectFilter} onChangeText={setSubjectFilter} style={styles.rangeInput} placeholder="All subjects" placeholderTextColor="#98A2B3" /></View></View><View style={styles.filters}><View style={styles.filter}><MaterialIcons name="calendar-today" size={14} color={colors.navy} /><Text style={styles.filterText}>{startDate} to {endDate}</Text></View><View style={styles.filter}><Text style={styles.filterText}>{subjectFilter || "All subjects"}</Text><MaterialIcons name="filter-list" size={16} color={colors.navy} /></View></View><View style={styles.recordSummary}><View><Text style={styles.summaryValue}>75%</Text><Text style={styles.summaryLabel}>Present today</Text></View><View style={styles.summaryLine} /><View><Text style={styles.summaryValue}>3 / 4</Text><Text style={styles.summaryLabel}>Records finalised</Text></View></View><Title>Session records</Title><RecordLine name="Aarav Patil" description="Bluetooth detected" isPresent /><RecordLine name="Isha Deshmukh" description="Bluetooth detected" isPresent /><RecordLine name="Rohan Jadhav" description="Faculty override" isPresent /><RecordLine name="Neha Shinde" description="Not marked" isPresent={false} /><View style={styles.dailyReportCard}><RoundIcon name="summarize" background="#F5EEDB" color="#9A6C0D" /><View style={styles.flex}><Text style={styles.promptTitle}>Full-day absence report</Text><Text style={styles.smallMuted}>Groups absent students by the selected class, subject, date range, lecture, room, and teacher.</Text></View></View>{filteredAbsences && filteredAbsences.length === 0 ? <Text style={styles.rangeStatus}>No absences match the selected report filters.</Text> : null}{absencesQuery.isFetching ? <Text style={styles.rangeStatus}>Refreshing report range…</Text> : null}<Button label="Export selected-range PDF" icon="picture-as-pdf" tone="gold" onPress={exportDailyPdf} /><View style={styles.gap} /><Button label="Export this lecture PDF" icon="picture-as-pdf" tone="outline" onPress={exportPdf} /><View style={styles.gap} /><Button label="Export CSV" icon="table-chart" tone="outline" onPress={exportCsv} /></>;
}
...
const styles = StyleSheet.create({
  ...
  input: { height: 52, backgroundColor: "#FAFAFA", borderWidth: 1.2, borderColor: colors.border, borderRadius: 14, paddingHorizontal: 14, fontSize: 14, color: colors.ink, marginBottom: 14 }, messageInput: { height: 120, paddingTop: 14 },
  ...
  bottom: { height: 74, backgroundColor: "#FFFFFF", borderTopWidth: 1, borderTopColor: colors.border, flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 32, alignItems: "center", shadowColor: colors.shadow, shadowOffset: { width: 0, height: -3 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 8 }, navItem: { minWidth: 58, alignItems: "center", gap: 4, paddingTop: 4 }, navText: { color: colors.muted, fontSize: 10, fontWeight: "800" }, navActiveIndicator: { width: 5, height: 5, borderRadius: 2.5, backgroundColor: colors.gold, marginBottom: -2 },
  ...
  authScreen: { flexGrow: 1, backgroundColor: colors.paper, paddingHorizontal: 20, paddingTop: 72, paddingBottom: 32 }, authScreenNarrow: { paddingHorizontal: 14 }, authBack: { position: "absolute", top: 18, left: 16, width: 44, height: 44, borderRadius: 22, backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: colors.border, alignItems: "center", justifyContent: "center", shadowColor: colors.shadow, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 3, zIndex: 10 }, authMark: { alignSelf: "center", width: 88, height: 88, borderRadius: 44, backgroundColor: colors.navy, alignItems: "center", justifyContent: "center", marginBottom: 18, shadowColor: colors.navy, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.35, shadowRadius: 16, elevation: 10 }, authCollegeLogo: { width: 60, height: 60 }, authTitle: { color: colors.ink, fontSize: 26, fontWeight: "900", textAlign: "center", letterSpacing: -0.5, marginBottom: 4 }, authDetail: { color: colors.muted, fontSize: 13, lineHeight: 19, textAlign: "center", marginTop: 6, marginBottom: 18 }, authCard: { backgroundColor: "#FFFFFF", borderWidth: 1, borderColor: colors.border, borderRadius: 22, padding: 16, shadowColor: colors.shadow, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 16, elevation: 4 }, authLink: { alignSelf: "center", paddingVertical: 14 }, authLinkText: { color: colors.maroon, fontSize: 13, fontWeight: "900" },
